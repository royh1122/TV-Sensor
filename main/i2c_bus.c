...
esp_err_t i2c_read_reg(uint8_t slave_addr, uint8_t reg_addr, uint16_t len, uint8_t *data) {
    esp_err_t ret;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (slave_addr<<1), 0x1);//ACK_CHECK_EN);
    i2c_master_write_byte(cmd, reg_addr, 0x1);//ACK_CHECK_EN);
    i2c_master_stop(cmd);
    ...
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (slave_addr<<1)|1, 0x1);//ACK_CHECK_EN);
    i2c_master_read(cmd, data, len, (i2c_ack_type_t)0x1);//I2C_MASTER_LAST_NACK);
    i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(I2C_NUM_0, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    return ret;
...
esp_err_t i2c_write_reg(uint8_t slave_addr, uint8_t reg_addr, uint16_t len, uint8_t *data) {
    esp_err_t ret;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    // first, send device address (indicating write) & register to be written
    i2c_master_write_byte(cmd, (slave_addr << 1 ) | 0, 0x1);//ACK_CHECK_EN);
    // send register we want
    i2c_master_write_byte(cmd, reg_addr, 0x1);//ACK_CHECK_EN);
    // write the data
    i2c_master_write(cmd, data, len, 0x1);//ACK_CHECK_EN);
    i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(I2C_NUM_0, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    return ret;
